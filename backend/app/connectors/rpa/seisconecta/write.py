...
class SeisConectaWriteConnector(ConfiguredWriteConnector):
    ...
    def _run_live_upsert_worker(
        ...
        account_id = str(account.get("source_platform_account_id") or "")
        if not entry_url or not account_id:
            return ConnectorResult(
                status="blocked_account_configuration_missing",
                message="6conecta: falta URL de entrada o cuenta para escritura live.",
        ...
        artifacts_dir = PROJECT_ROOT / "artifacts" / "external-writes" / "seisconecta"
            ...
            json.dumps(
                {
                    ...
                    "nationality": str(worker_metadata.get("nationality") or ""),
        ...
        command = [
            sys.executable,
            str(PROJECT_ROOT / "scripts" / "seisconecta_live_upsert_worker.py"),
            "--entry-url",
            entry_url,
            "--account-id",
            account_id,
            ...
            "--platform-label",
            f"6conecta / {account.get('external_company_name') or account_id}",
            "--target-context",
        ...
        try:
            completed = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=int(worker_metadata.get("live_timeout_seconds") or 1200),
                check=False,
            )
        finally:
        ...
        external_write_executed = bool(stdout_payload.get("external_write_executed") or payload.get("external_write_executed"))
        ...
        if completed.returncode == 0 and external_write_executed and post_write_read_confirmed:
            ...
            message = "6conecta: alta enviada y confirmada por lectura posterior."
            ...
            external_status = "confirmed"
            message = "6conecta: el trabajador ya existe en la plataforma; alta duplicada bloqueada y lectura confirmada."
            ...
            message = "6conecta: alta enviada, pendiente de confirmacion por lectura posterior."
        else:
            status = helper_state if helper_state != "unknown" else "live_write_not_completed"
            external_status = "not_synced"
            message = f"6conecta: escritura live no completada ({status})."
        ...
        return ConnectorResult(
            ...
            | {
                ...
                "persist_external_status": external_write_executed or post_write_read_confirmed,
